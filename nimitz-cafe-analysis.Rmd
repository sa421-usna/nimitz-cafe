...
by customers after placing their orders. In particular, we measure the simulated average delay customers experience at the barista queue with 1, 2, 3, and 4 baristas. By understanding the impact on the barista queue delay, we focus on the part of NCB directly affected by the number of baristas working. Because we focus on the barista queue, we do not model the self service coffee dispenser.
...
We modeled the NCB in JaamSim. A graphical representation of the simulation model is shown below. We did not model the self-service coffee dispenser as our focus was delay at the barista queue.
...
We suggest NCB pursue the following to determine between 2, 3, or 4 baristas:
  ...
  -Include the self service coffee line to the simulation model, especially if overall customer experience is analyzed.
