...
denscomp(list(interarrival.fit.exp, interarrival.fit.gamma, interarrival.fit.lnorm), 
         legendtext = c('exponential', 'gamma', 'lognormal'), plotstyle = 'ggplot')
```
...
```{r echo = FALSE}
denscomp(list(cashier.fit.gamma, cashier.fit.lnorm), legendtext = c('gamma', 'lognormal'), plotstyle = 'ggplot')
```
...
```{r echo = FALSE}
cdfcomp(list(cashier.fit.gamma, cashier.fit.lnorm), legendtext = c('gamma', 'lognormal'), plotstyle = 'ggplot')
```
...
```{r echo = FALSE}
denscomp(list(service.fit.gamma, service.fit.lnorm), legendtext = c('gamma', 'lognormal'), plotstyle = 'ggplot')
```
...
```{r echo = FALSE}
cdfcomp(list(service.fit.gamma, service.fit.lnorm), legendtext = c('gamma', 'lognormal'), plotstyle = 'ggplot')
```
...
Based on the results presented, increasing the number of baristas from 1 to 2 has the most significant impact, reducing the predicted average delay experienced by customers after placing their orders from around 2 minutes to 10 seconds, i.e., by a factor of roughly 12 times. Increasing the number of baristas further also decreases the predicted delay, although perhaps not as dramatically. The prediction reliability also increases significantly with added baristas. The reliability of these predictions imply these estimates have a worst case variability of 27% of the predicted mean.
